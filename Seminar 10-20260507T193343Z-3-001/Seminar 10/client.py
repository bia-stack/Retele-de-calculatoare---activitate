...
class FTPClient:
    ...
    def read_file(self):
        ...
        # Get list of files from server first
        list_command = {'command': 'list_files'}
        list_response = self.send_command(list_command)
        
        if list_response['status'] != 'success':
            print(f"✗ Could not retrieve file list: {list_response['message']}")
            return
    ...
    def see_file_operation_history(self):
        ...
        if response['status'] == 'error':
            ...
            if history:
                ...
                print(f"✓ {response.get('message', 'No history available')}")
