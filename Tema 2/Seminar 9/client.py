...
while True:
    ...
    if comanda == 'EXIT':
        ...
        if not este_conectat:
            print("EROARE: Trebuie sa fii conectat pentru a publica un mesaj.")
            continue
        
        parti_pub = intrare.split(' ', 1)
        if len(parti_pub) < 2 or not parti_pub[1].strip():
        ...
        if not este_conectat:
            print("EROARE: Trebuie sa fii conectat pentru a sterge un mesaj.")
            continue
            
        parti_del = intrare.split(' ', 1)
        if len(parti_del) < 2:
        ...
        try:
            ...
            print("EROARE: ID-ul furnizat pentru DELETE trebuie sa fie un numar intreg.")
            continue
